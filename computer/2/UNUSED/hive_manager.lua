...
function Main()
    ...
    for _, attached_peripheral in pairs(peripherals) do
        ...
        if string.find(attached_peripheral, vars.fuges) then
            ...
            -- PUSH HONEY TO HONEY STORAGE VESSELS
            for _, honeybucket in pairs(honey_storage) do
                honey_collected = honey_collected + container.pushFluid(honeybucket)
                print(attached_peripheral, honeybucket)
                -- local honeybucket = 'basicFluidTank_1'
            end
            ...
            for slot, item in pairs(container.list()) do
                ...
                elseif string.find(item.name, 'productivebees:wither_') then
                    ...
                    totalWarehousedThisRun = totalWarehousedThisRun +
                    whi.DepositInAnyWarehouse(attached_peripheral, slot)
                elseif string.find(item.name, 'productivebees:obsidian_shard') then
                    -- SEND TO WAREHOUSE
                    totalWarehousedThisRun = totalWarehousedThisRun +
                    whi.DepositInAnyWarehouse(attached_peripheral, slot)
